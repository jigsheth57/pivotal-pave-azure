...
{
	if [ $OS == "Linux" ]; then
		PIVNET_FILE="https://github.com/pivotal-cf/pivnet-cli/releases/download/v2.0.1/pivnet-linux-amd64-2.0.1"
	elif [ $OS == "Darwin" ]; then
		PIVNET_FILE="https://github.com/pivotal-cf/pivnet-cli/releases/download/v2.0.1/pivnet-darwin-amd64-2.0.1"
	fi
...
function get_clis()
{
  $CWD/pivnet login --api-token=$1
	if [ $OS == "Linux" ]; then
		$CWD/pivnet download-product-files --product-slug='pivotal-container-service' --release-version='1.8.2' --product-file-id=737302
    $CWD/pivnet download-product-files --product-slug='pivotal-container-service' --release-version='1.8.2' --product-file-id=792042
    $CWD/pivnet download-product-files --product-slug='pivotal-container-service' --release-version='1.8.2' --product-file-id=737305
    mv pks-linux-amd64-* pks && chmod 755 pks
    mv kubectl-linux-amd64-* kubectl && chmod 755 kubectl
    mv tkgi-linux-amd64-* tkgi && chmod 755 tkgi
	elif [ $OS == "Darwin" ]; then
		$CWD/pivnet download-product-files --product-slug='pivotal-container-service' --release-version='1.8.2' --product-file-id=737301
    $CWD/pivnet download-product-files --product-slug='pivotal-container-service' --release-version='1.8.2' --product-file-id=792041
    $CWD/pivnet download-product-files --product-slug='pivotal-container-service' --release-version='1.8.2' --product-file-id=737304
    mv pks-darwin-amd64-* pks && chmod 755 pks
    mv kubectl-darwin-amd64-* kubectl && chmod 755 kubectl
    mv tkgi-darwin-amd64-* tkgi && chmod 755 tkgi
	fi
}
